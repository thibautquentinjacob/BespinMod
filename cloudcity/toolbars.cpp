...
#undef adjustConditionally
    ...
    else if (isEnabled && (option->state & State_On))
    {
		...
		QRectF leftRectangleWhite( 0, 0, 20, rect.height() );
		QRectF clipLeftWhite( 0, 0, 10, rect.height()+1 );
		painter->setClipRect( clipLeftWhite );
		painter->drawRoundRect( leftRectangleWhite, 45, 45 );
		painter->setClipping(false);
		QRectF RightRectangleWhite( rect.width()-20, 0, 19, rect.height() );
		QRectF clipRightWhite( rect.width()-10, 0, 10, rect.height()+1 );
		painter->setClipRect( clipRightWhite );
		painter->drawRoundRect( RightRectangleWhite, 45, 45 );
		painter->setClipping(false);
		painter->drawLine( 10, 0, rect.width()-11, 0 ); // White top line
		painter->drawLine( 10, rect.height(), rect.width()-11, rect.height() ); // White bottom line
    
		painter->setPen( QColor( 0, 0, 0, 100 ) ); // Black color
		painter->drawLine( 11, 1, rect.width()-11, 1 ); // Black top line
		painter->drawLine( 11, rect.height()-1, rect.width()-11, rect.height()-1 ); // Black bottom line
    	QRectF leftRectangleBlack( 1, 1, 19, rect.height()-2 );
    	QRectF clipLeftBlack( 1, 1, 10, rect.height() );
    	painter->setClipRect( clipLeftBlack );
    	painter->drawRoundRect( leftRectangleBlack, 45, 45 );
    	painter->setClipping(false);
    	QRectF RightRectangleBlack( rect.width()-21, 1, 19, rect.height()-2 );
    	QRectF clipRightBlack( rect.width()-10, 1, 10, rect.height() );
